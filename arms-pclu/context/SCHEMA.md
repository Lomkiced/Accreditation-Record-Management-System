...
erDiagram
    ...
    Indicator {
        ...
        string confidential_docs "JSON array of specific confidential evidence items"
        datetime created_at
        datetime updated_at
    }
    ...
    Document {
        ...
        boolean is_archived "Soft delete from active views"
        boolean is_deleted_by_faculty "Permanent removal from faculty view while retained in repository"
        boolean is_archived_from_repo "Archived from Dean/Admin active repository"
        datetime created_at
        datetime updated_at
    }
...
### 4. Multi-Tier Soft-Delete & Institutional Evidence Preservation
...
- **Indicators (`isConfidential`, `confidentialDocs`)**:
  ...
  - `confidentialDocs`: Stores a JSON array of specific required evidence items marked as confidential, allowing granular selection with a master "Select All" toggle in the indicator builder.
