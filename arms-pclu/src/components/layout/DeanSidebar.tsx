...
export function DeanSidebar({ user: serverUser }: DeanSidebarProps) {
  const pathname = usePathname()
  const { user: storeUser, signOut } = useAuth()
  const prefetch = usePrefetch()

  const displayName = serverUser?.name ?? storeUser?.name ?? "Dean"
  const displayRole = serverUser?.designation ?? storeUser?.designation ?? "Program Dean"
  ...
  return (
    <aside className="sidebar-fixed bg-[#0F172A] flex flex-col">
      <div className="h-16 flex items-center px-4 border-b border-slate-700/50 flex-shrink-0">
        <Link href="/dean/dashboard" className="flex items-center gap-2">
          <span className="text-xl font-bold text-white tracking-tight">ARMS</span>
          <span className="w-1.5 h-1.5 rounded-full bg-violet-500 mt-0.5"></span>
        </Link>
        <span className="text-xs text-slate-400 ml-auto">Dean</span>
      </div>
