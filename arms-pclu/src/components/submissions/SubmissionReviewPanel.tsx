...
export function SubmissionReviewPanel({ open, onClose, submission }: SubmissionReviewPanelProps) {
  ...
  const isReviewable = submission.status === "PENDING" || submission.status === "UNDER_REVIEW"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
            <div className="grid grid-cols-3 gap-2">
              ...
              <span className="col-span-2 text-sm text-slate-700 font-medium">{submission.faculty}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <span className="text-xs text-slate-400 uppercase tracking-wide">Area</span>
              <span className="col-span-2 text-sm text-slate-700 font-medium">Area {submission.area.number}: {submission.area.name}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <span className="text-xs text-slate-400 uppercase tracking-wide">Criterion</span>
              <span className="col-span-2 text-sm text-slate-700 font-medium">{submission.criterion}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <span className="text-xs text-slate-400 uppercase tracking-wide">Submitted</span>
              <span className="col-span-2 text-sm text-slate-700 font-medium">{submission.submittedAt}</span>
            </div>

            <div className="mt-4 bg-slate-50 border-l-4 border-slate-300 p-3 italic text-sm text-slate-600 rounded-r-lg">
              “Attached is the updated syllabus for the current academic year, addressing previous remarks.”
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">Syllabus_2024.pdf</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 px-4">
            <VersionHistory versions={submission.version > 1 ? [
              { version: 2, date: "Oct 12, 2024", status: "PENDING", remarks: null },
              { version: 1, date: "Oct 05, 2024", status: "RETURNED", remarks: "Please ensure all signatures are present." },
            ] : [
              { version: submission.version, date: submission.submittedAt, status: submission.status, remarks: null }
            ]} />
          </div>
          ...
          {isReviewable ? (
            <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-4 shadow-sm border-t-4 border-t-blue-500">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Button variant="outline" className="w-full border-red-500 text-red-600 hover:bg-red-50" onClick={onClose}>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2">
              ...
              <div className="text-sm text-slate-600">
                Reviewed by <span className="font-medium">Admin User</span> on Oct 13, 2024
              </div>
              {submission.status === "RETURNED" && (
                <div className="mt-2 text-xs text-red-600 bg-red-50 p-3 rounded italic">
                  “Please ensure all signatures are present.”
                </div>
              )}
            </div>
          )}
