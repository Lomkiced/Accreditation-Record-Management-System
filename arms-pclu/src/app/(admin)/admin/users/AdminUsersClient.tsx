...
export function AdminUsersClient({ initialData }: { initialData: UserWithCounts[] }) {
  ...
  const { data: archivedUsers = [], isLoading: isLoadingArchived } = useArchivedUsers(["ADMIN", "DEAN", "FACULTY"])
  ...
  }, [currentList, searchQuery, selectedDept, roleFilter])
  ...
  const handleArchiveConfirm = async () => {
    ...
    if (result && !result.success) {
      toast.error(result.error || "Failed to archive user.")
    } else {
      ...
      <div className="space-y-4">
        ...
        {(activeView === "active" ? isLoading && users.length === 0 : isLoadingArchived && archivedUsers.length === 0) ? (
          <div className="py-10 text-center text-slate-500 text-sm animate-pulse bg-white rounded-xl border border-slate-200">
            Loading {activeView === "active" ? "user accounts" : "archived users"}...
          </div>
        ) : activeView === "active" ? (
          <UsersTable 
            data={filteredUsers} 
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm py-16 text-center">
            ...
            <p className="text-base font-semibold text-slate-700">No archived users</p>
            <p className="text-sm text-slate-400 mt-1">When you archive a user, they will appear here.</p>
          </div>
        ) : (
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 bg-amber-50/50">
              <p className="text-xs font-medium text-amber-700">
                Archived users cannot log in. You can restore them or permanently delete them below.
              </p>
            </div>
            <ul className="divide-y divide-slate-100">
              {filteredUsers.map((user) => (
                <li key={user.id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors">
                  ...
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200 uppercase shrink-0">
                    {user.role}
                  </span>
