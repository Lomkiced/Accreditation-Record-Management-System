...
export function FacultyDashboardClient({
  ...
  const { data: assignments = [], isPending: pendingAssignments, isFetching: fetchingAssignments } = useAssignments(user?.id ?? "", initialAssignments)
  const { data: areas = [], isPending: pendingAreas, isFetching: fetchingAreas } = useAreas(initialAreas)
  const { data: submissions = [], isPending: pendingSubmissions, isFetching: fetchingSubmissions } = useMySubmissions(initialSubmissions)
